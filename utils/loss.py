...
class ComputeLoss:
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        sigmas = torch.tensor(
            [1.07, 1.07, 1.07, .35, .35, .79, .79, .72, .72, .62, .62, 1.07, 1.07, .87, .87, .89, .89][:self.nkpt],
            device=device) / 10.0
    ...
    def build_targets(self, p, targets):
        ...
        if self.kpt_label:
            gain = torch.ones(self.nkpt * 2 + 7, device=targets.device).long()
        ...
        for i in range(self.nl):
            ...
            if self.kpt_label:
                gain[2:self.nkpt * 2 + 7 - 1] = torch.tensor(p[i].shape)[(self.nkpt * 2-1) * [3, 2]]  # xyxy gain
            else:
